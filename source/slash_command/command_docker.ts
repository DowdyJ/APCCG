...
export default class CommandHello extends ApccgSlashCommand {
    ...
    public CommandData(): any {
        ...
        .addSubcommand(subcommand => 
            subcommand
                ...
                .addStringOption((input) => input.setRequired(true).setName('name').setDescription("short name for the command ([a-zA-Z0-9_]{3,20})"))
                .addStringOption((input) => input.setRequired(true).setName('command').setDescription("command to run (without --name)"))
                .addStringOption((input) => input.setRequired(false).setName('notes').setDescription("Server IP, other notes")))
        .addSubcommand(subcommand => 
            subcommand
                .setName("remove")
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addBooleanOption((input) => input.setRequired(true).setName('can_alter_users').setDescription("add/remove user permission"))
                .addBooleanOption((input) => input.setRequired(true).setName('can_start_containers').setDescription("start container permission"))
                .addBooleanOption((input) => input.setRequired(true).setName('can_stop_containers').setDescription("stop container permission"))
                .addBooleanOption((input) => input.setRequired(true).setName('can_add_commands').setDescription("add command permission"))
                .addBooleanOption((input) => input.setRequired(true).setName('can_remove_commands').setDescription("remove command permission")))
        .addSubcommand(subcommand =>
            subcommand
                .setName("de-op")
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName("status")
                .setDescription("Show container status")
        );
    }
  ...
  private async addUser(interaction: CommandInteraction): Promise<boolean> {
    ...
    let res = await database.addDockerUser(userToChange.id, canAlterUsers, canAddCommands, canRemoveCommands, canRunCommands, canStopCommands);
  ...
  private async addCommand(interaction: CommandInteraction): Promise<boolean> {
    ...
    if (res)
        ...
        interaction.reply(`Error adding command \"${commandName}\" as \"${commandString}\"`);
  ...
  private async removeCommand(interaction: CommandInteraction): Promise<boolean> {
    ...
    if (res)
        ...
        interaction.reply(`Error removing command \"${commandName}\"`)
  ...
  private async getDockerStatus(interaction: CommandInteraction): Promise<boolean> {
    ...
    let activeContainers = (await this.getRunningContainers()).filter((entry) => {return commandNames.includes(entry)})
