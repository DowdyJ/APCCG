...
**/salty status** -> Fetch current match details, including current bet amounts and team names.`;
  ...
  CommandData(): any {
      ...
      .addSubcommand(subcommand => 
        subcommand
          .setName("sign-out")
          .setDescription("sign out from SaltyBet"))      
      .addSubcommand(subcommand => 
        subcommand
          .setName("bet")
          ...
          .addNumberOption((input) => input.setRequired(true).setName("betamount").setDescription("amount to bet (-1 for all-in)")))
      .addSubcommand(subcommand =>
        subcommand
          .setName("balance")
      ...
      .addSubcommand(subcommand =>
        subcommand
          .setName("status")
          .setDescription("log SaltyBet match data to channel")
      );
  }
  ...
  async LogCurrentMatchInfo(interaction : discord.CommandInteraction) : Promise<boolean> {
  ...
  async CreateEmbedForSaltyInfo() : Promise<discord.EmbedBuilder> {
    ...
    let ratioString : string = +activeTeamsData.p1total === 0 || +activeTeamsData.p2total === 0 ? "N/A" : (+activeTeamsData.p1total > +activeTeamsData.p2total ? `${(+activeTeamsData.p1total/+activeTeamsData.p2total).toFixed(2)} : 1` : `1 : ${(+activeTeamsData.p2total/+activeTeamsData.p1total).toFixed(2)}`);
    ...
    .addFields(
      { name: 'Stats', value: `Odds: ${ratioString}\nStatus: ${modeStatus}` },
      { name: `${activeTeamsData?.p1name}`, value: `\$${activeTeamsData.p1total}`, inline: true },
      { name: '\u200B', value: '\u200B', inline: true },
      { name: `${activeTeamsData?.p2name}`, value: `\$${activeTeamsData.p2total}`, inline: true },
      { name: '\u200B', value: '\u200B', inline: false },
    )

    .setTimestamp()
    .setFooter({ text: `${activeTeamsData.remaining}`, iconURL: 'https://www.saltybet.com/images/SALTYBETheader3.png' });
  }

  async SignOut(interaction : discord.CommandInteraction) {
    return new Promise<boolean>((resolve, reject) => 
    {
      ...
      curl.on('end', function(statusCode, data, headers) {
        ...
        if (statusCode !== 302) {
          ...
          interaction.reply({ephemeral: true, content:"Failed to log out."})
          return false;
        }

        interaction.reply({ephemeral: true, content:"Successfully logged out."})
        this.close();
        resolve(true);
      });
  ...
  async VoteForTeam(interaction : discord.CommandInteraction, teamNumber : number, amount : number) : Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
        ...
        if (currentBalance > amount || amount === -1)
